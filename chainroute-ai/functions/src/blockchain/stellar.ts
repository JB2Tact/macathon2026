...
export async function getStellarFee(): Promise<{ fee: number; time: string }> {
  try {
    const feeStats = await server.feeStats();
    const baseFee = parseInt(feeStats.last_ledger_base_fee, 10);
    // Convert stroops to USD (approximate)
    const feeInXLM = baseFee / 10_000_000;
    const feeInUSD = feeInXLM * 0.12; // approximate XLM price
